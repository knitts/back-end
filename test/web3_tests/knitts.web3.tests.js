...
describe('Knitts', async function(){
    ...
    it('should register an user', async()=>{
        ...
        user = await user.deploy({data:User.bytecode, arguments: [userAddress, "Arvinth"]}).send({from:userAddress});

        await knitts.methods.register(accounts[1], user.address).send();
...
describe('Knitts-League', async()=>{
    ...
    it('should allow users to register', async()=>{
        for(let i=0; i<6 ; i++){
            ...
            user = await new web3.eth.Contract(User.abi, {from:organization});
            user = await knitts.deploy({data:User.bytecode, arguments:[accounts[1], "Arvinth"]}).send({from:accounts[1], gas: gasfee}).then((instance)=>{
                return instance;
            });
            await knitts.methods.register(accounts[i], _user.address).send({ from: organization});
        }
    });
    it('should allow moderator to create a league', async()=>{
        await knitts.methods.createLeague(_entryFee, _numPlayers, _duration).send( {from:moderator, value:_deposit});
        var numLeagues = await knitts.methods.numLeagues().call();
        var leagueAddress = await knitts.methods.Leagues(numLeagues-1).call();
        league = await web3.eth.Contract(League.abi, (leagueAddress));
